...
namespace asimov
{
    public partial class Form3 : Form
    {
        ...
        private async void cbListeEleve_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cbListeEleve.SelectedItem != null)
            {
                ...
                try
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        string responseBody = await response.Content.ReadAsStringAsync();
                        var jsonResponse = JsonConvert.DeserializeObject<dynamic>(responseBody);
                        var listeScolarite = JsonConvert.DeserializeObject<List<Scolarite>>(jsonResponse.scolarite.ToString());
                        ...
                        foreach (var scolarite in listeScolarite)
                        {
                            dt.Rows.Add(scolarite.nom, scolarite.libelle_classe,scolarite.numero_semestre, scolarite.moyenne_semestre, scolarite.annee_scolaire,scolarite.id_scolarite);
                        }
